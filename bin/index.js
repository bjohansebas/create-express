...
const program = new Command(packageJson.name)
  ...
  .option('--cjs', 'Use CommonJS modules.')
  .option('--example <name>', 'Starter example to use (minimal, api, web, mvc).')
  .option('--view <engine>', 'View engine to use (none, ejs, pug, handlebars).')
  .option('--linter <name>', 'Linter to use (none, biome, oxlint, eslint).')
  .option('--test <runner>', 'Test runner to use (none, vitest, node, mocha).')
  .option('--docker', 'Add a Dockerfile.')
  .option('--no-docker', 'Do not add a Dockerfile.')
